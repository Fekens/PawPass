...
function dashboard(){
  const today=state.tasks.filter(t=>t.date==="Today"), done=today.filter(t=>t.done).length, p=selectedPet(), name=p?.name||"your pet";
...
document.addEventListener("click",e=>{
  ...
  const deletePet=e.target.closest("[data-pet-delete]"); if(deletePet){ e.stopPropagation(); const p=state.pets.find(x=>String(x.id)===deletePet.dataset.petDelete); if(p&&confirm(`Delete ${p.name}'s profile? This cannot be undone.`)){ state.pets=state.pets.filter(x=>x!==p); if(String(state.selectedPetId)===String(p.id))state.selectedPetId=state.pets[0]?.id??null; save();render();toast(`${p.name}'s profile deleted`); } }
