...
function petCard(p, manage=false){ const active=String(p.id)===String(state.selectedPetId); return `<article class="card pet-card ${active?"selected-pet":""}" data-pet-select="${p.id}"><div class="pet-card-top"><span>${petVisual(p)}</span><b class="health-badge">${active?"✓ Selected":"● "+esc(p.status||"Profile ready")}</b></div><div class="pet-details"><div><h3>${esc(p.name)}</h3><p>${esc(p.breed)} · ${esc(p.species||"")}</p></div><div class="pet-stats"><div><b>${esc(displayAge(p))}</b><small>AGE</small></div><div><b>${esc(p.weight)}</b><small>WEIGHT</small></div></div></div>${manage?`<footer class="pet-actions">${isPetLost(p)?`<button class="link-btn reunited-link" data-mark-home="${p.id}">✓ Mark as Home</button>`:""}<button class="link-btn" data-pet-edit="${p.id}">Edit</button><button class="link-btn danger" data-pet-delete="${p.id}">Delete</button></footer>`:""}</article>`; }
